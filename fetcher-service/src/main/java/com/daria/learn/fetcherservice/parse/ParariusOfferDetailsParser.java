...
public class ParariusOfferDetailsParser extends ParariusParser implements OfferDetailParser  {
    ...
    private static final Map<String, RentalStatusDTO> statusMap =
            Map.of("for rent", RentalStatusDTO.AVAILABLE,
                    "under option", RentalStatusDTO.UNDER_OPTION,
                    "deleted", RentalStatusDTO.DELETED);

    @Override
    public DetailRentalOffersDTO parseOfferDetailsDTO(Element rootElement, String link) {
        ...
        RentalStatusDTO statusDTO = getStatus(rootElement).orElse(RentalStatusDTO.OTHER);
        ...
        return new DetailRentalOffersDTO(name, link, statusDTO, postalCode,
    ...
    private Optional<RentalStatusDTO> getStatus(Element rootElement) {
